...
build :: (Ord a) => [a] -> BST a
build [] = Leaf
build (x:xs) = Node x ls rs
    where ls = build [l | l <- xs, l < x]
          rs = build [r | r <- xs, x < r]
...
contains x (Node t l r ) = (x == t) || contains x l || contains x r
...
add x h@(Node y l r) | y > x = Node y (add x l) r
                     | y == x = Node y l r
...
removeMin (Node x l r) = (fst (removeMin l), Node x (snd (removeMin l)) r)
removeMin (Node x' (Node x Leaf r ) r') = (x, Node x' r r')

-------------------------------------------------------------------------------
-- | Removing an element
-------------------------------------------------------------------------------

helper :: (Ord a) => BST a -> BST a
helper (Node _ Leaf Leaf) = Leaf
helper (Node _ (Node x' l r) Leaf) = Node x' l r
helper (Node _ Leaf (Node x' l r)) = Node x' l r
helper (Node _ l r) = Node (fst (removeMin r)) l (snd (removeMin r))

remove :: (Ord a) => a -> BST a -> BST a
remove _ Leaf = Leaf
remove x t@(Node x' l r) | x == x' = helper t 
                         | x > x' = Node x' l (remove x r)
                         | otherwise = Node x' (remove x l) r  
...
prop_multiset []=True
prop_multiset xs = forall (build xs) (\x -> contains x (build xs))
